...
class FileUploadBehavior extends \yii\base\Behavior
{
    ...
    public function beforeValidate()
    {
        ...
        if ($this->owner->{$this->attribute} instanceof UploadedFile) {
			...
			\Yii::warning('Exit 1');
			\Yii::warning($this->file);
            return;
        }
		
		\Yii::warning('Upload');
		\Yii::warning($this->owner);
		\Yii::warning($this->owner->image);
		\Yii::warning($this->attribute);
    ...
    public function beforeSave()
    {
        if ($this->file instanceof UploadedFile) {
            ...
            $this->owner->{$this->attribute} = Yii::$app->getSecurity()->generateRandomString(16) . '.' . $this->file->extension;
    ...
    public function getUploadedFileUrl($attribute)
    {
        ...
        $behavior = static::getInstance($this->owner, $attribute);
        return \yii\helpers\Url::to($behavior->resolvePath($behavior->fileUrl));
    }
}

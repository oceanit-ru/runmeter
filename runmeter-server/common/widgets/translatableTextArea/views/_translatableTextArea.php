...
<?php
	...
	$attributeLabel = $model->getAttributeLabel($attribute);
					echo $form->field($model->translate($sourceLang), "[$sourceLang]$attribute", [
						...
						. "</div>\n"
						. "{hint}\n"
						. "{error}"])->label(isset($label) ? $label : $attributeLabel . " [$sourceLang]")->textarea(['rows' => '3', 'style' => 'resize:vertical', 'placeholder' => "Введите $attributeLabel ..."]);
	Modal::begin([
		'id' => "$modalName",
		'options' => [
		...
		foreach ($languageList as $language) {
			...
			if (Yii::$app->sourceLanguage !== $lang) {
				echo $form->field($model->translate($lang), "[$lang]$attribute",['enableClientValidation' => false])->label($language['label'] . " [$lang]")->textarea(['rows' => '3', 'style' => 'resize:none']);
			}
		}
	Modal::end();
?>
